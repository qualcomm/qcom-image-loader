...
<tr><td><code>--reset-device</code></td><td style="white-space: nowrap;"><code>--device</code></td><td style="white-space: nowrap;"><code>--verbose</code></td><td>Reset device from firehose mode. Normally used when the previous command was executed with <code>--reset=false</code>.</td></tr>
...
1. **ADB Cannot Run Simultaneously with QIL**

   QIL and ADB cannot access the same USB device simultaneously. Only one application can hold a handle to a USB device interface at a time. If ADB is running, stop it before using QIL:
